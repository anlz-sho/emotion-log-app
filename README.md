...
Ranking 画面に集計単位の切り替えを追加しました。

- 全期間 / 週別 / 月別 / 曜日別を segmented control で切り替え
- 既存の感情ランキング表示を維持
- 既存の感情グループ別ランキング表示を維持
- 週別は選択中の週に含まれるログで集計
- 月別は選択中の月に含まれるログで集計
- 曜日別は選択中の曜日に該当するログで集計
- 左右ボタンで週 / 月 / 曜日を切り替え
- 既存の Ranking 画面 UI を維持

### Mac/Xcodeでビルド確認する手順

Task 6 のビルド確認は、あとで MacBook + Xcode で実施してください。

1. MacBook でこのリポジトリを pull する
2. `EmotionLogApp.xcodeproj` を Xcode で開く
3. iOS 17 以上の Simulator を選択する
4. Ranking タブに All / Week / Month / Weekday の segmented control が表示されることを確認する
5. 各集計単位に切り替えても、感情ランキングとグループランキングが表示されることを確認する
6. Week / Month / Weekday で左右ボタンを押すと集計対象が切り替わることを確認する
7. 回数順、回数表示、グループ色の表示が維持されていることを確認する
